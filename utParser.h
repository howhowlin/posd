...
TEST_F(ParserTest, ListAsStruct) {
  Scanner scanner(".(1,[])");
  Parser  parser(scanner);
  Struct *ss = static_cast<Struct*>(parser.createTerm());
  EXPECT_EQ(2,ss->arity());
  EXPECT_EQ(".(1, [])",ss->symbol());
  EXPECT_EQ("1",ss->args(0)->symbol());
  EXPECT_EQ("[]",ss->args(1)->symbol());
}
//
//
...
TEST_F(ParserTest, ListAsStruct2) {
  ...
  Struct *ss = static_cast<Struct*>(parser.createTerm());
 EXPECT_EQ(2,ss->arity());
  EXPECT_EQ(".(2, .(1, []))",ss->symbol());
  EXPECT_EQ("2",ss->args(0)->symbol());
  EXPECT_EQ(".(1, [])",ss->args(1)->symbol());
